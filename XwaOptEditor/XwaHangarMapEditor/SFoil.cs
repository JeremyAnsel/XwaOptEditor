...
namespace XwaHangarMapEditor
{
    public sealed class SFoil
    {
        ...
        public static IList<SFoil> GetSFoilsList(string optFilename)
        {
            ...
            var hatchesLines = GetSFoilsHatchesLines(ship);

            var lines = new List<string>(sfoilsLines.Count + landingGearsLines.Count + hatchesLines.Count);
            lines.AddRange(sfoilsLines);
            lines.AddRange(landingGearsLines);
            lines.AddRange(hatchesLines);
